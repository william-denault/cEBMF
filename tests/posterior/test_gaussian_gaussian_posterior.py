import pytest
import numpy as np 
from cebmf.routines.numerical_routine import apply_log_sum_exp 
...
from cebmf.routines.distribution_operation import get_data_loglik_normal, convolved_logpdf_normal
...
def test_convolved_loglik_postmean():
    ...
    n=betahat.shape[0]
    p= scale.shape[0]
    convolved= convolved_logpdf_normal(betahat[1],sebetahat[1], location,scale)
    ...
    np.testing.assert_allclose(convolved, expected_convolved , rtol=1e-5)
    np.testing.assert_allclose(res, expected_res , rtol=1e-5)
    np.testing.assert_allclose(data_loglik , expected_loglik_mat , rtol=1e-5)
    assert np.isclose(post_mean[0], 0.23635376, atol=1e-7)
    assert np.isclose(post_mean2[0], 0.39779246, atol=1e-7)
    assert np.isclose(out.post_mean[0], 0.23635376, atol=1e-7)
    assert np.isclose(out.post_mean2[0], 0.39779246, atol=1e-7)
